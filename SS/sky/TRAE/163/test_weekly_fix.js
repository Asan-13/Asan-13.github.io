...
function getNextOccurrence(task, now) {
    try {
        ...
        if (task.recurrence.type === "weekly") {
            ...
            console.log("转换后的weekdays数组:", weekdays);
            ...
            for (let i = 0; i < 7; i++) {
                ...
                console.log(`检查日期 ${checkDate.toISOString().split('T')[0]} (星期${checkWeekday}) 是否在weekdays中`);
                
                if (weekdays.includes(checkWeekday)) {
                    baseDate = checkDate;
                    found = true;
                    console.log(`找到匹配的日期: ${baseDate.toISOString().split('T')[0]}`);
                    break;
                }
            }
...
console.log(`每周几：${task.recurrence.weekdays}`);

// 执行测试
const result = getNextOccurrence(task, now);
if (result.startTime) {
    console.log(`计算结果：${result.startTime.toLocaleString()}`);
    console.log(`计算日期：${result.startTime.toISOString().split('T')[0]}`);
    
    // 验证结果
    const expectedDate = "2025-12-28";
    const resultDate = result.startTime.toISOString().split('T')[0];
    if (resultDate === expectedDate) {
        console.log("✅ 测试通过：正确计算出下一次发生日期为28日");
    } else {
        console.log(`❌ 测试失败：期望日期为${expectedDate}，实际计算结果为${resultDate}`);
    }
} else {
